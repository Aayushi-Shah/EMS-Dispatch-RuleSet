...
CRITICAL: We restrict to
- unit_type in {AMB, MEDIC, MICU}
- AND station_number that exists in the station GeoJSON (has lat/lon).
...
def main():
    ...
    ap.add_argument(
        "--stations",
        type=str,
        default="reference/maps/lemsa_stations_points.geojson",
        help="Station points GeoJSON (default: reference/maps/lemsa_stations_points.geojson)",
    )
    ap.add_argument(
        "--out",
        type=str,
    ...
    rows = []
    for _, row in calls[[incident_col, units_col]].iterrows():
        units_str = row[units_col]
        incident_id = row[incident_col]
        for u in extract_unit_tokens(units_str):
            rows.append({"incidentID": incident_id, "unit_designator": u})
    ...
    if units.empty:
        ...
        urban_union = _read_union("reference/maps/urban_areas_lancaster.geojson")
        rural_union = _read_union("reference/maps/rural_areas_lancaster.geojson")
        county_union = _read_union("reference/maps/lancaster_county_boundary.geojson")
    ...
    # -----------------------------
    # 4) Save canonical units table as CSV
    # -----------------------------
